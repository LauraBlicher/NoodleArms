...
public class SteamTrail : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		for (int i = 1; i < currentNumberOfPoints; i++ ) {
			...
			tempVec = tempVec + directions[i] * Time.deltaTime;
			positions[i] = tempVec;
	...
	public Vector3 getSmokeVec()
	{
		...
		Vector3 smokeVec = new Vector3();
		smokeVec = new Vector3(Random.Range(-1f,1f), Random.Range(0f,1f), Random.Range(-1f, 1f)).normalized * spread;

		//smokeVec = smokeVec * spread;
		smokeVec.y = smokeVec.y + riseSpeed;
